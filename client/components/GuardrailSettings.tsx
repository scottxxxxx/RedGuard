...
const GuardrailSettings = forwardRef<GuardrailSettingsHandle, Props>(function GuardrailSettings({ onConfigChange, onBotConfigUpdate, botConfig, isConnected }, ref) {
    ...
    const fetchedSnapshotRef = useRef<{ toggles: typeof toggles; bannedTopics: string; regexPatterns: string } | null>(null);
    ...
    const applyAnalysisResult = useCallback((data: any) => {
        ...
        // Save snapshot of fetched state for comparison
        fetchedSnapshotRef.current = {
            toggles: { ...newToggles },
            bannedTopics: data.topics?.length > 0 ? data.topics.join(', ') : bannedTopics,
            regexPatterns: data.regexPatterns?.length > 0 ? data.regexPatterns.join('\n') : regexPatterns
        };
    }, []);
        ...
        try {
            ...
            pollIntervalRef.current = setInterval(async () => {
                try {
                    ...
                    // Stop polling on HTTP errors (4xx/5xx)
                    if (!statusRes.ok) {
                        if (pollIntervalRef.current) clearInterval(pollIntervalRef.current);
                        pollIntervalRef.current = null;
                        const errData = await statusRes.json().catch(() => ({}));
                        setBackupStatus('error');
                        setBackupError(errData.error || `Backup check failed (${statusRes.status})`);
    ...
    const matchesFetchedSnapshot = useCallback((newToggles: typeof toggles, newTopics: string, newRegex: string) => {
        const snap = fetchedSnapshotRef.current;
        if (!snap) return false;
        const togglesMatch = Object.keys(newToggles).every(
            k => newToggles[k as keyof typeof newToggles] === snap.toggles[k as keyof typeof snap.toggles]
        );
        return togglesMatch && newTopics === snap.bannedTopics && newRegex === snap.regexPatterns;
    }, []);

    const updateGuardrailSource = useCallback((newToggles: typeof toggles, newTopics: string, newRegex: string) => {
        if (guardrailSource !== 'synced' && guardrailSource !== 'modified') return;
        setGuardrailSource(matchesFetchedSnapshot(newToggles, newTopics, newRegex) ? 'synced' : 'modified');
    }, [guardrailSource, matchesFetchedSnapshot]);

    const handleToggle = (key: keyof typeof toggles) => {
        setToggles(prev => {
            const updated = { ...prev, [key]: !prev[key] };
            updateGuardrailSource(updated, bannedTopics, regexPatterns);
            return updated;
        });
    };

    return (
        <div className="card p-6 h-full flex flex-col overflow-hidden relative group">
            <div className="flex flex-col h-full overflow-y-auto pr-2 -mr-2">
                ...
                {guardrailSource === 'fetching' && (
                    <div className="mb-4 p-3 rounded-lg text-xs flex items-center gap-3 bg-blue-50/80 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 border border-blue-200/60 dark:border-blue-800/50 backdrop-blur-md shadow-sm transition-all duration-300">
                ...
                {guardrailSource === 'synced' && (
                    <div className="mb-4 p-3 rounded-lg text-xs flex items-center gap-3 bg-emerald-50/80 dark:bg-emerald-900/20 text-emerald-800 dark:text-emerald-300 border border-emerald-200/60 dark:border-emerald-800/50 backdrop-blur-md shadow-sm transition-all duration-300">
                        <svg className="w-4 h-4 shrink-0 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                        </svg>
                        <span className="flex-1 font-medium">Guardrails synced with bot</span>
                    </div>
                )}
                {guardrailSource === 'modified' && (
                    <div className="mb-4 p-3 rounded-lg text-xs flex items-center gap-3 bg-amber-50/80 dark:bg-amber-900/20 text-amber-800 dark:text-amber-300 border border-amber-200/60 dark:border-amber-800/50 backdrop-blur-md shadow-sm transition-all duration-300">
                        ...
                        <span className="flex-1 font-medium">Configuration modified — may not reflect actual bot guardrails</span>
                    </div>
                )}
                {guardrailSource === 'error' && backupError && (
                    <div className="mb-4 p-3 rounded-lg text-sm flex items-start gap-3 bg-red-50/90 dark:bg-red-900/20 text-red-700 dark:text-red-200 border border-red-200/80 dark:border-red-800/50 backdrop-blur-md shadow-sm transition-all duration-300">
                        ...
                        <div className="flex-1">
                            <span className="font-semibold">Fetch from Bot failed</span>
                            <p className="mt-1 text-xs text-red-600/80 dark:text-red-300/80 font-mono">{backupError}</p>
                        </div>
                        <button
                            onClick={() => { setGuardrailSource('default'); setBackupError(null); setBackupStatus('idle'); }}
                            className="text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-200 transition-colors p-1 -mr-1 -mt-1 rounded-md hover:bg-red-100/50 dark:hover:bg-red-900/50"
                            title="Dismiss"
                        >
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div className="space-y-2 border-t border-[var(--border)] pt-3">
                        ...
                        {(toggles.topics_input || toggles.topics_output) && (
                            <div className="ml-2 mt-2">
                                <textarea
                                    value={bannedTopics}
                                    onChange={(e) => { setBannedTopics(e.target.value); updateGuardrailSource(toggles, e.target.value, regexPatterns); }}
                    ...
                    <div className="border-t border-[var(--border)] pt-3">
                        ...
                        {toggles.regex && (
                            <div className="ml-9 mt-2">
                                <textarea
                                    value={regexPatterns}
                                    onChange={(e) => { setRegexPatterns(e.target.value); updateGuardrailSource(toggles, bannedTopics, e.target.value); }}
