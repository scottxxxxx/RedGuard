...
export default function BatchTester({ botConfig, guardrailConfig }: Props) {
    ...
    return (
        <div className="bg-[var(--surface)] shadow rounded-lg p-6 h-[600px] flex flex-col">
            <h3 className="text-lg font-medium text-[var(--foreground)] mb-4 border-b border-[var(--border)] pb-2">
            ...
            <div className="mb-4 space-y-4">
                <div className="flex gap-4 items-end">
                    <div className="flex-1">
                        ...
                        <input
                            ...
                            className="block w-full text-sm text-[var(--foreground-muted)] file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-[var(--primary-50)] file:text-[var(--primary-700)] hover:file:bg-[var(--primary-100)]"
                        />
                        <p className="text-xs text-[var(--foreground-muted)] mt-1">Format: conversation_number, utterance</p>
                    </div>
                    <button
                        onClick={processBatch}
                        disabled={!file || isProcessing || !guardrailConfig}
                        className="bg-[var(--primary-600)] text-white px-6 py-2 rounded-md hover:bg-[var(--primary-700)] disabled:opacity-50 h-10"
                    >
                ...
                {isProcessing && (
                    <div className="w-full bg-[var(--surface-hover)] rounded-full h-2.5 border border-[var(--border)]">
                        <div className="bg-[var(--primary-600)] h-2.5 rounded-full" style={{ width: `${progress}%` }}></div>
                    </div>
                )}
            </div>

            <div className="flex-1 h-0 overflow-auto border border-[var(--border)] rounded-md">
                <table className="min-w-full divide-y divide-[var(--border)]">
                    <thead className="bg-[var(--surface-hover)] sticky top-0">
                        <tr>
                            <th className="px-3 py-2 text-left text-xs font-medium text-[var(--foreground-muted)] uppercase tracking-wider">ID</th>
                            <th className="px-3 py-2 text-left text-xs font-medium text-[var(--foreground-muted)] uppercase tracking-wider">Messages</th>
                            <th className="px-3 py-2 text-left text-xs font-medium text-[var(--foreground-muted)] uppercase tracking-wider">Status</th>
                            <th className="px-3 py-2 text-left text-xs font-medium text-[var(--foreground-muted)] uppercase tracking-wider">Details</th>
                        </tr>
                    </thead>
                    <tbody className="bg-[var(--surface)] divide-y divide-[var(--border)]">
                        {results.map((res, i) => (
                            <tr key={i} className={res.status === 'pass' ? 'bg-success-bg' : 'bg-error-bg'}>
                                <td className="px-3 py-2 whitespace-nowrap text-sm font-medium text-[var(--foreground)]">
                                    {res.conversationId}
                                </td>
                                <td className="px-3 py-2 text-sm text-[var(--foreground-muted)] max-w-xs truncate">
                                    <div className="font-semibold">User: {res.utterance}</div>
                                    <div className="text-xs">Bot: {res.botResponse.substring(0, 50)}...</div>
                                </td>
                                <td className="px-3 py-2 whitespace-nowrap">
                                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full border ${res.status === 'pass' ? 'bg-success-bg text-success-text border-success-border' : 'bg-error-bg text-error-text border-error-border'
                                        }`}>
                                        {res.status.toUpperCase()}
                                    </span>
                                </td>
                                <td className="px-3 py-2 text-sm text-[var(--foreground-muted)]">
                                    {res.failedGuardrail || '-'}
                                </td>
                            </tr>
                        ))}
                        {results.length === 0 && !isProcessing && (
                            <tr>
                                <td colSpan={4} className="px-3 py-8 text-center text-sm text-[var(--foreground-muted)]">
            ...
            {results.length > 0 && (
                <div className="mt-4 flex justify-end">
                    <button
                        onClick={downloadCsv}
                        className="text-[var(--primary-600)] hover:text-[var(--primary-700)] text-sm font-medium flex items-center gap-2"
                    >
